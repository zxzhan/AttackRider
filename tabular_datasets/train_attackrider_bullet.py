...
def load_args():
    ...
    args['Bullet'] = {
        ...
        'step_size_prime': args['AT']['step_size']*3,
        'gamma': 0.5,
    }
    ...
    if args['ridersize'] != -1:
        args['output'] = './output/' + args['dataset'] + '/' + args['algorithm']  + '_e' + str(args['ridersize']) + '_seed' + str(args['seed'])
    
    if args['epsilon'] != 0.1:
        args['output'] = './output/' + args['dataset'] + '/' + args['algorithm']  + '_e' + str(args['ridersize']) + '_ep' + str(args['AT']['epsilon']) + '_seed' + str(args['seed'])
        if args['gamma'] != 0.5:
            args['output'] = './output/' + args['dataset'] + '/' + args['algorithm']  + '_e' + str(args['ridersize']) + '_ep' + str(args['AT']['epsilon']) + '_gamma' + str(args['Bullet']['gamma']) + '_seed' + str(args['seed'])
...
if __name__ == "__main__":
    ...
    for epoch in stream.epochs(args['training']['n_epochs']):
        print_epoch_info()
        if stream.epoch!=1: 
            print("F_B = {:.4f} | F_R = {:.4f} | F_O = {:.4f}".format(F_B.item(), F_R.item(), F_O.item()))
        ...
        for ii, batch_idx in enumerate(epoch):
            ...
            n = int(torch.ceil(args['AtkRider']['m_prime'] / (p_percent * args['training']['batch_size']) ).item())
                
            if batch_count % n == 1: # the first batch
            ...
            loss, (batch_acc, batch_err), (F_O, F_B, F_R), model_update_times = lib.train_at_atkrider_batch(
                optimizer,
                loss_fn,
                model=model,
